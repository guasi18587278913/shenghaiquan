...
export function PostCard({ post, onLikeUpdate, onUpdate }: PostCardProps) {
  ...
  const handleBookmark = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      // 更新收藏数（如果后端返回了收藏数）
      if (data.bookmarkCount !== undefined && onUpdate) {
        onUpdate({
          ...post,
          _count: {
            ...post._count,
          ...
          <div className="flex items-center gap-1 -ml-2">
            ...
            >
              ...
              {post._count?.bookmarks > 0 && (
                <span className="text-sm font-medium">{post._count.bookmarks}</span>
              )}
            </button>
          </div>
        </div>
